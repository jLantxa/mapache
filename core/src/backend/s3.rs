...
impl S3Backend {
    pub fn new(
        ...
        let force_path_style = if let Some(endpoint_url) = &endpoint {
            config_loader = config_loader.endpoint_url(endpoint_url);
            // Force path style for non-AWS endpoints (e.g., MinIO)
            !endpoint_url.contains("amazonaws.com")
        } else {
            false
        };

        let config = runtime.block_on(config_loader.load());
        let client_builder = aws_sdk_s3::config::Builder::from(&config)
    ...
    fn path_from_key(&self, key: &str) -> Result<PathBuf> {
        ...
        path.strip_prefix(&self.prefix)
            .map(|p| p.to_path_buf())
            .map_err(|_| anyhow!("Key '{}' is not under prefix '{}'", key, self.prefix.display()))
    }
}

fn make_key(prefix: &Path, path: &Path) -> String {
    let mut full_path = prefix.to_path_buf();
    full_path.push(path);
    // S3 keys should not start with /
    full_path.to_string_lossy().trim_start_matches('/').to_string()
}
...
impl StorageBackend for S3Backend {
    fn create(&self) -> Result<()> {
        // We assume the bucket exists. 
        // We could try to create it, but usually credentials provided are limited.
        // Instead, we just check if we can list objects (check connectivity).
        let bucket_exists = self.runtime.block_on(self.client.head_bucket().bucket(&self.bucket).send()).is_ok();
        
        if !bucket_exists {
             return Err(anyhow!("Bucket '{}' does not exist or is not accessible.", self.bucket));
        }

        // Create marker file
        let marker_path = Path::new(".mapache_initialized");
        self.write(&Handle::new(marker_path), b"")?;
    ...
    fn path_exists(&self, path: &Path) -> bool {
        ...
        let file_exists = self.runtime.block_on(
            self.client
                .head_object()
                .bucket(&self.bucket)
                .key(&key)
    ...
    fn read(&self, handle: &Handle, offset: isize, length: usize) -> Result<Vec<u8>> {
        ...
        if offset != 0 || length != 0 {
            let range = if offset < 0 {
                 ...
                 let head = self.runtime.block_on(self.client.head_object().bucket(&self.bucket).key(&key).send())
                    .context("Failed to head object for relative seek")?;
        ...
        let output = self.runtime.block_on(req.send())
            .with_context(|| format!("Failed to read object '{}'", key))?;

        let data = self.runtime.block_on(output.body.collect())
            .context("Failed to collect body data")?
            .into_bytes();
    ...
    fn rename(&self, from: &Path, to: &Path) -> Result<()> {
        ...
        self.runtime.block_on(
            self.client
                ...
                .key(&dest_key)
                .send()
        ).context("Failed to copy object during rename")?;
        ...
        self.runtime.block_on(
            self.client
                ...
                .key(&src_key)
                .send()
        ).context("Failed to delete source object during rename")?;
         ...
         loop {
             ...
             if let Some(objects) = list_resp.contents {
                 if !objects.is_empty() {
                     ...
                     for obj in objects {
                         if let Some(k) = obj.key {
                             delete_ids.push(aws_sdk_s3::types::ObjectIdentifier::builder().key(k).build()?);
                         }
                     }
    ...
    fn is_file(&self, path: &Path) -> bool {
         ...
         self.runtime.block_on(self.client.head_object().bucket(&self.bucket).key(&key).send()).is_ok()
    }

    fn is_dir(&self, path: &Path) -> bool {
        let key = self.key_from_path(path);
        // If key is empty, it means we are at the root of the prefix/bucket
        if key.is_empty() { return true; } 

        // Check if it's not a file and has contents with prefix
