...
pub async fn run(global_args: &GlobalArgs, args: &CmdArgs) -> Result<(), CatError> {
    with_repository_lock(
        ...
        new_backend_with_prompt(global_args.backend_options(false))
            .await
            .map_err(|e| CatError::ObjectNotFound(format!("Failed to initialize backend: {e}")))?,
            ...
            match &args.object {
                ...
                Object::Pack(prefix) => {
                    ...
                    let object = repo.load_pack(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load object: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
                ...
                Object::Tree(prefix) => {
                    ...
                    let tree = repo.load_blob(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load tree blob: {e}"))
                    })?;
                    let tree: Tree =
                ...
                Object::Blob(prefix) => {
                    ...
                    let blob = repo.load_blob(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load blob: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
                ...
                Object::Index(prefix) => {
                    ...
                    let index = repo.load_index(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load index: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
                ...
                Object::Key(prefix) => {
                    ...
                    let key = repo.load_key(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load key: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
                ...
                Object::Snapshot(prefix) => {
                    ...
                    let snapshot = repo.load_snapshot(&id, None).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load snapshot: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
                ...
                Object::Lock(prefix) => {
                    ...
                    let lock = repo.load_lock(&id).await.map_err(|e| {
                        CatError::ObjectNotFound(format!("Failed to load lock: {e}"))
                    })?;
                    ui::cli::log!(
                        "{}",
