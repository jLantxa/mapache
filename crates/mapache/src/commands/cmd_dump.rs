    ...
    with_repository_lock(
        ...
        |repo, _, lock_handle| async move {
            ...
            let (_snapshot_id, snap) = find_use_snapshot(repo.clone(), &args.snapshot)
                .await
                .map_err(|_| DumpError::SnapshotNotFound("Snapshot not found".to_string()))?
                .ok_or_else(|| DumpError::SnapshotNotFound("Snapshot not found".to_string()))?;

            let node = find_serialized_node(repo.as_ref(), &snap.tree, &args.path)
                .await?
                .ok_or_else(|| DumpError::PathNotFound("Path not found".to_string()))?;
            ...
            if let Some(blob_ids) = &node.blobs {
                for blob_id in blob_ids {
                    let data = repo.load_blob(blob_id).await.map_err(|e| {
                        DumpError::DumpFailed(format!("Failed to load blob {blob_id}: {e}"))
                    })?;
