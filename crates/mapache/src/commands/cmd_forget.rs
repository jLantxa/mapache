...
pub enum ForgetError {
    ...
    ForgetFailed(String),
    #[error("forget interrupted by user: {0}")]
    Interrupted(String),
    #[error(transparent)]
...
async fn forget_phase(
    ...
    let mut snapshots_sorted: SnapshotEntryList = SnapshotStream::new(repo.clone())
        .await
        .map_err(|e| ForgetError::ForgetFailed(format!("Failed to load snapshots: {e}")))?
        ...
        for entry in &removed_snapshots {
            ...
            if args.force {
                repo.delete_file(ContentIdType::Snapshot, &entry.id, None)
                    .await
                    .map_err(|e| {
                        ForgetError::ForgetFailed(format!("Failed to delete snapshot: {e}"))
                    })?;
            } else {
                repo.set_extension(
                ...
                .await
                .map_err(|e| {
                    ForgetError::ForgetFailed(format!("Failed to mark snapshot for deletion: {e}"))
                })?;
            }
        }
    }
    ...
    if cleanup_handler.is_interrupted() {
        ...
        return Err(ForgetError::Interrupted(
            "Forget interrupted by user.".to_string(),
        ));
    }
