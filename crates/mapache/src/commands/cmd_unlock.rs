...
pub async fn run(global_args: &GlobalArgs, args: &CmdArgs) -> Result<(), UnlockError> {
    let backend = new_backend_with_prompt(global_args.backend_options(false))
        .await
        .map_err(|e| UnlockError::RepoOpenFail(format!("Failed to initialize backend: {e}")))?;
    ...
    .await
    .map_err(|e| UnlockError::RepoOpenFail(format!("Failed to open repository: {e}")))?;

    let locks = repo
        .get_locks()
        .await
        .map_err(|e| UnlockError::RepoOpenFail(format!("Failed to get locks: {e}")))?;
    ...
    for lock in locks {
        if args.force || lock.is_expired() {
            ...
            repo.delete_file(ContentIdType::Lock, lock.id(), None)
                .await
                .map_err(|e| UnlockError::RepoOpenFail(format!("Failed to delete lock: {e}")))?;
            num_deleted_locks += 1;
        }
    }
