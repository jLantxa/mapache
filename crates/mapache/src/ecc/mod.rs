...
/// Parsed ECC sidecar header fields.
struct EccHeader {
    k: usize,
    p: usize,
    original_len: usize,
...
pub(crate) fn ecc_decode(data: &[u8], ecc_payload: &[u8]) -> Result<Vec<u8>, EccDecodeError> {
    let hdr = parse_header(ecc_payload).ok_or(EccDecodeError::InvalidHeader)?;

    if data.len() != hdr.original_len || hdr.k == 0 || hdr.p == 0 || hdr.k + hdr.p > 256 {
        return Err(EccDecodeError::InvalidHeader);
    }

    let layouts = calculate_stripe_layouts(hdr.original_len, hdr.k, hdr.p);
    if layouts.len() != hdr.stripe_count {
        return Err(EccDecodeError::InvalidHeader);
    }
    ...
    let rs = ReedSolomon::new(hdr.k, hdr.p).map_err(|_| EccDecodeError::InvalidHeader)?;
    ...
    let expected_id = ID::from_bytes(hdr.protected_hash);
    if ID::from_content(&out) != expected_id {
...
pub(crate) fn validate_crc(data: &[u8], ecc_payload: &[u8]) -> Result<(), usize> {
    let hdr = match parse_header(ecc_payload) {
        Some(h) => h,
        None => return Err(0),
    };
    ...
    if ID::from_content(data) != ID::from_bytes(hdr.protected_hash) {
        return Err(0);
    }

    let layouts = calculate_stripe_layouts(hdr.original_len, hdr.k, hdr.p);

    // Verify the payload is exactly the size generated by ecc_encode so
    // that every CRC read below stays in bounds and a bloated or truncated
    // sidecar is rejected the same way ecc_decode rejects it.
    let mut expected_size = HEADER_SIZE;
    for s in &layouts {
        expected_size += stripe_payload_size(s.data_shards, s.parity_shards);
    }
